...
export default function CourseDetailsPage() {
  ...
  const handleEnroll = () => {
    if (!user) {
      navigate(`/auth?mode=register&courseId=${courseId}`);
    } else if (!isEnrolled) {
      navigate(`/auth?mode=register&courseId=${courseId}`);
    }
  };
      ...
      <div className="mt-6">
        {isEnrolled ? (
          <Link
            ...
            className="inline-block px-6 py-3 text-sm font-medium rounded-md bg-success text-success-foreground"
          >
            Visit Course
          </Link>
        ) : (
          <button
            ...
            className="px-6 py-3 text-sm font-medium rounded-md bg-primary text-primary-foreground"
          >
            Enroll Now
          </button>
        )}
