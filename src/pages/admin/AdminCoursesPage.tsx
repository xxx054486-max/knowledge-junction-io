...
export default function AdminCoursesPage() {
  ...
  const resetForm = () => {
    ...
    setDiscussionGroups([{ name: "", link: "" }]); setRoutinePDF(""); setEditCourse(null);
  };
  ...
  const openEdit = (c: Course) => {
    ...
    setRoutinePDF(c.routinePDF || ""); setDialogOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = {
        ...
        routinePDF, createdAt: Timestamp.now(),
      };
  ...
  return (
    <div className="p-4 animate-fade-in">
      ...
      <div className="space-y-3">
        {courses.map((c) => (
          <div key={c.id} className="p-3 bg-card rounded-lg border border-border flex gap-3">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <input type="text" placeholder="Course Name" value={courseName} onChange={(e) => setCourseName(e.target.value)} required className="w-full px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm" />
            ...
            <input type="number" placeholder="Price" value={price} onChange={(e) => setPrice(Number(e.target.value))} className="w-full px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm" />
            ...
            <div>
              ...
              {instructors.map((inst, i) => (
                <div key={i} className="flex flex-col gap-1 mt-2 p-2 bg-accent rounded-md">
                  ...
                  <input value={inst.image} onChange={(e) => { const a = [...instructors]; a[i] = { ...a[i], image: e.target.value }; setInstructors(a); }} placeholder="Image URL" className="px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm" />
                  {instructors.length > 1 && <button type="button" onClick={() => setInstructors(instructors.filter((_, j) => j !== i))} className="text-xs text-destructive self-end">Remove</button>}
                </div>
              ))}
              <button type="button" onClick={() => setInstructors([...instructors, { name: "", subject: "", image: "" }])} className="text-xs text-primary mt-1">+ Add Instructor</button>
            </div>
            ...
            <div>
              ...
              {discussionGroups.map((g, i) => (
                <div key={i} className="flex gap-1 mt-1">
                  <input value={g.name} onChange={(e) => { const a = [...discussionGroups]; a[i] = { ...a[i], name: e.target.value }; setDiscussionGroups(a); }} placeholder="Group Name" className="flex-1 px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm" />
                  <input value={g.link} onChange={(e) => { const a = [...discussionGroups]; a[i] = { ...a[i], link: e.target.value }; setDiscussionGroups(a); }} placeholder="Link" className="flex-1 px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm" />
            ...
            <input type="text" placeholder="Routine PDF URL" value={routinePDF} onChange={(e) => setRoutinePDF(e.target.value)} className="w-full px-3 py-2 rounded-md bg-background border border-border text-foreground text-sm" />
