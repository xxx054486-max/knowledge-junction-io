      ...
      {courses.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {courses.map((course) => (
            <div
              key={course.id}
              className="bg-card rounded-lg shadow-card overflow-hidden border border-border"
            >
              {course.thumbnail ? (
                <img
              ...
              <div className="p-4">
                ...
                <Link
                  to={`/course/${course.id}`}
                  className="inline-block mt-3 px-4 py-2 text-sm font-medium rounded-md bg-primary text-primary-foreground"
                >
                  View Details
                </Link>
              </div>
